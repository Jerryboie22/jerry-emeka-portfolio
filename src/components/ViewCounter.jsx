...
export default function ViewCounter() {
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-40 flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full glass shadow-premium font-mono text-[10px] sm:text-xs max-w-[calc(100vw-2rem)]"
    >
      <Eye size={13} className="text-blue flex-shrink-0" />
      <span className="text-text font-semibold">{views.toLocaleString()}</span>
      <span className="text-muted">
        <span className="hidden sm:inline">portfolio </span>views
      </span>
    </motion.div>
  );
}
